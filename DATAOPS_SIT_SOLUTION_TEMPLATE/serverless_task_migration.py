...
def migrate_task(task_name, wh_name, wh_size):
    with st.container(key="migrate_container"):
        ...
        st.write(f"from warehouse {wh_name}, and make it serverless with the below setting")
        wh_size_list = ["XSMALL","SMALL","MEDIUM","LARGE","XLARGE","XXLARGE","XXXLARGE"]
        wh_size = st.selectbox("Initial Warehouse Size",wh_size_list,index=wh_size_list.index(wh_size.replace('-','').upper()))
        ...
        if col2.button("Migrate", type="primary"):
            ...
            session.sql(f"ALTER TASK {task_name} SET USER_TASK_MANAGED_INITIAL_WAREHOUSE_SIZE = 'XSMALL';").collect()
            session.sql(f"""ALTER TASK {task_name} SET COMMENT = '{{"origin": "sf_sit","name": "sit_serveless_task_migration","version": "{{major: 1, minor: 0}}"}}'""").collect()
...
if "qual_tasks" not in st.session_state:
    with st.spinner("Fetching Tasks"):
        ...
        st.session_state.qual_tasks = session.sql("""SELECT DISTINCT CONCAT(t.DATABASE_NAME,'.',t.SCHEMA_NAME,'.', NAME) as FULL_NAME,NAME, t.QUERY_TEXT, WAREHOUSE_NAME, WAREHOUSE_SIZE, iff(DATEDIFF("MINUTE",QUERY_START_TIME, COMPLETED_TIME) < 1, 'TRUE','FALSE') as SHORT_RUNTIME, iff(DATEDIFF("MINUTE",QUERY_START_TIME, SCHEDULED_TIME) > 1, 'TRUE','FALSE') as RAN_OVER FROM SNOWFLAKE.ACCOUNT_USAGE.TASK_HISTORY t
          ...
          AND NOT CONTAINS(WAREHOUSE_NAME,'COMPUTE_SERVICE') 
          AND SCHEDULED_TIME BETWEEN dateadd(DAY, - 14, CURRENT_TIMESTAMP()) AND CURRENT_TIMESTAMP();""").to_pandas()

qual_tasks = st.session_state.qual_tasks

with st.container(border=True):
    st.write("""The tasks below are the first glance of tasks that may qualify for migration to serverless tasks \n
    They either run for under a minute, or run longer than the time between their scheduled run and the next run
    """)
    st.dataframe(qual_tasks, use_container_width=True, column_config={'FULL_NAME':None})

with st.container(border=True):
    st.write("""The Tasks that run long in the previous list are great candidates for serverless warehouses as they can have a target finish set and the warehouse will scale for the schedule""")
    st.write("""For short running tasks, to further qualify them we'll check how many tasks generally run on the same warehouse""")

with st.container(border=True):
    st.write("""The below list shows the number of tasks that all run around the same time on each warehouse""")
    ...
    st.write("For any tasks that run for less than a minute on a warehouse alone should be migrated to a serverless warehouse")
    st.write("Additionally any tasks scheduled with a total runtime of less than a few minutes would likely each benefit from being serverless")
    st.write("Review each warehouse below and migrate any tasks that are recommended or that you would like to")
    
    for wh_name in warehouses:
        wh_tasks = warehouse_list[warehouse_list["WAREHOUSE_NAME"] == wh_name]["NAME"].tolist()
        wh_size = qual_tasks[qual_tasks["WAREHOUSE_NAME"] == wh_name]["WAREHOUSE_SIZE"].values[0]
        with st.expander(wh_name):
            ...
            for wh_task in wh_tasks:
                full_task_name = warehouse_list[warehouse_list["NAME"] == wh_task]["FULL_NAME"].values[0]
                describe_task = pd.DataFrame(session.sql(f"DESCRIBE TASK {full_task_name}").collect())
                if describe_task["warehouse"].values[0]:
                    st.divider()
                    col1,__,col2 = st.columns(3)
                    col1.write(wh_task)
                    col2.button("Migrate",on_click=migrate_task, args=(full_task_name,wh_name,wh_size), key=wh_task+"migrate_button")
